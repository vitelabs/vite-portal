...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		logger.Logger().Trace().Err(err).Msg("RPC client reconnect failed")
		return err
	}
	select {
...
func (c *Client) dispatch(codec ServerCodec) {
	...
	for {
		...
		case err := <-c.readErr:
			conn.handler.extendLogger(logger.Logger()).Debug().Err(err).Msg("RPC connection read error")
			conn.close(err, lastOp)
			reading = false

		// Reconnect:
		case newcodec := <-c.reconnected:
			logger.Logger().Debug().Bool("reading", reading).Str("conn", newcodec.RemoteAddr()).Msg("RPC client reconnected")
			if reading {
