...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*Server, net.Listener) {
		...
		go http.Serve(l, srv.WebsocketHandler([]string{"*"}, nil, nil, nil))
		return srv, l
	}
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client, *httptest.Server) {
	...
	switch transport {
	case "ws":
		hs = httptest.NewUnstartedServer(srv.WebsocketHandler([]string{"*"}, nil, nil, nil))
	case "http":
...
func (l *flakeyListener) Accept() (net.Conn, error) {
	...
	if err == nil {
		...
		time.AfterFunc(timeout, func() {
			logger.Logger().Debug().Msg(fmt.Sprintf("killing conn %v after %v", c.LocalAddr(), timeout))
			c.Close()
		})
	}
	return c, err
