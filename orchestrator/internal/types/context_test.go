...
func TestProcessedSet(t *testing.T) {
	...
	c := NewContext(cfg)
	cc1, err := c.GetChainContext(sharedtypes.DefaultSupportedChains[0].Name)
	require.NoError(t, err)
	cc2, err := c.GetChainContext(sharedtypes.DefaultSupportedChains[1].Name)
	require.NoError(t, err)
	store1 := cc1.GetStatusStore()
	store2 := cc2.GetStatusStore()
	require.NoError(t, err)
